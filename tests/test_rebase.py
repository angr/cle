...
@unittest.skipUnless(pypcode is not None, "pypcode not installed")
def test_rebase_uses_the_mapped_address_width():
    """A DOS image has 16-bit registers but can place objects above 64 KiB."""
    path = os.path.join(TEST_BASE, "tests", "i386", "dos", "hello.exe")
    ld = cle.Loader(path, auto_load_libs=False, main_opts={"base_addr": 0x90000, "force_rebase": True})
    assert ld.main_object.arch.bits == 16
    assert ld.main_object.mapped_address_bits == 20

    extern = ld.extern_object
    assert extern.min_addr > ld.main_object.max_addr
    assert extern.max_addr < 2**20
    assert ld.find_object_containing(0) is None
    ld.memory.unpack_word(extern.min_addr, size=1)
...
def test_null_page_is_used_when_the_address_space_has_nothing_else():
    ...
    """
    path = os.path.join(TEST_BASE, "tests", "i386", "rcr_test")
    ld = cle.Loader(
        path,
        auto_load_libs=False,
        page_size=0x1000,
        main_opts={"backend": "blob", "arch": archinfo.ArchPcode("z80:LE:16:default"), "base_addr": 0x4000},
    )
...
def test_null_guard_tracks_the_loader_page_size():
    ...
    address space is one such page.
    """
...
@unittest.skipUnless(pypcode is not None, "pypcode not installed")
def test_null_guard_has_no_extra_floor():
    """The configured page size is the complete null-page guard, even when it is one byte."""
    path = os.path.join(TEST_BASE, "tests", "i386", "rcr_test")
    ld = cle.Loader(
        path,
        auto_load_libs=False,
    ...
    main = ld.main_object
    above = MockBackend(2**16 - (main.max_addr + 1), arch=main.arch)
    ld.dynamic_load(above)
    assert (above.min_addr, above.max_addr) == (main.max_addr + 1, 2**16 - 1)

    below = MockBackend(0x100, arch=main.arch)
    ld.dynamic_load(below)
    assert below.min_addr == 1
    assert ld.find_object_containing(0) is None


@unittest.skipUnless(pypcode is not None, "pypcode not installed")
def test_image_inside_the_null_guard_does_not_bypass_it():
    """Searching above a low image must still prefer space above the guard."""
    path = os.path.join(TEST_BASE, "tests", "i386", "rcr_test")
    for page_size, granularity in ((0x2000, 0x100000), (0x1000, 1)):
        ld = cle.Loader(
            path,
            auto_load_libs=False,
            page_size=page_size,
            rebase_granularity=granularity,
            main_opts={"backend": "blob", "arch": archinfo.ArchPcode("z80:LE:16:default"), "base_addr": 0},
        )
        assert ld.main_object.max_addr < page_size
        extern = ld.extern_object
        assert extern.min_addr >= page_size
        assert extern.max_addr < 2**16
        assert ld.find_object_containing(page_size - 1) is None
        ld.memory.unpack_word(extern.min_addr, size=1)
...
if __name__ == "__main__":
    ...
    test_rebase_uses_the_mapped_address_width()
    test_null_page_is_used_when_the_address_space_has_nothing_else()
    test_null_guard_tracks_the_loader_page_size()
    test_null_guard_has_no_extra_floor()
    test_image_inside_the_null_guard_does_not_bypass_it()
