...
def test_image_inside_the_null_guard_does_not_bypass_it():
    ...
    for page_size, granularity in ((0x2000, 0x100000), (1, 1)):
        ld = cle.Loader(
            path,
            auto_load_libs=False,
            page_size=page_size,
        ...
        guard = max(page_size, 0x1000)
        assert ld.main_object.max_addr < guard
        extern = ld.extern_object
        assert extern.min_addr >= guard
        assert extern.max_addr < 2**16
        assert ld.find_object_containing(guard - 1) is None
        ld.memory.unpack_word(extern.min_addr, size=1)
