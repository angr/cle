...
class IMAGE_REL_BASED_HIGHADJ(PEReloc):
    ...
    def value(self):
        ...
        """
        org_bytes = self.owner_obj.memory.load(self.relative_addr, 2)
...
class IMAGE_REL_BASED_HIGHLOW(PEReloc):
    @property
    def value(self):
        org_bytes = self.owner_obj.memory.load(self.relative_addr, 4)
        org_value = struct.unpack('<I', org_bytes)[0]
...
class IMAGE_REL_BASED_DIR64(PEReloc):
    @property
    def value(self):
        org_bytes = self.owner_obj.memory.load(self.relative_addr, 8)
