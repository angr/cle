...
class Coff(Backend):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for section_idx, section in enumerate(self._coff.sections):
            ...
            if section.PointerToRawData or not vsize:
                vaddr = section.PointerToRawData
                filesize = vsize
            else:
                # A section with no bytes in the file -- .bss and anything else marked
                # IMAGE_SCN_CNT_UNINITIALIZED_DATA -- states PointerToRawData 0, which in the layout above is the
                # file header. Placing it there lays it over the header and, once it is longer than the section
                # table, over the sections that follow: a mingw object whose .bss is 0x1300 bytes long covers the
                # first 0x11fc bytes of its own .text. Give it zero-filled space of its own past the image instead.
                alignment = _section_alignment(section)
                vaddr = (len(image) + alignment - 1) & ~(alignment - 1)
                image.extend(bytes(vaddr + vsize - len(image)))
                filesize = 0
