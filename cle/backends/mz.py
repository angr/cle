...
class MZHeader:
    ...
    @property
    def image_paragraphs(self) -> int:
        return (self.image_size + _PARAGRAPH_SIZE - 1) // _PARAGRAPH_SIZE

    @property
    def minimum_allocation_size(self) -> int:
        """The memory DOS must find before it will start the program."""
        return (self.image_paragraphs + self.minimum_extra_paragraphs) * _PARAGRAPH_SIZE

    @property
    def maximum_allocation_size(self) -> int:
        """The memory DOS gives the program when it has that much to spare."""
...
class MZ(Backend):
    ...
    def __init__(self, *args, **kwargs):
        ...
        extension = self._recognized_extension(reader)
    ...
    def is_compatible(cls, stream):
        ...
        try:
            ...
            if cls._recognized_extension(reader) is not None:
                return False
    ...
    @staticmethod
    def _recognized_extension(reader: _Reader) -> bytes | None:
        # e_lfanew lives at 0x3c whatever the header paragraph count says, and PE images exist whose
        # stub declares a two-paragraph header and still carries a real pointer there, so this must
        # not be gated on the declared header size.
        if reader.size < _EXTENDED_HEADER_POINTER_END:
            return None
        ...
        if pointer < _FIXED_HEADER_SIZE or pointer > reader.size - 2:
            return None
