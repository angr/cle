...
class ELFCore(ELF):
    ...
    def __reload_children(self):
        ...
        for filename, patches in child_patches.items():
            ...
            # iterate over all the core segments, since the only time we will need
            # to make a change to the child's memory is if the core has something to say about it
            # if there is ANY OVERLAP AT ALL, copy over the relevant data and nuke the segment
            # then, if there is any part of the segment which DOESN'T correspond to a child segment,
            # inject a new memory backer into the child for the relevant data
            ...
            while i < len(remaining_segments):
                ...
                if base_addr <= seg.vaddr <= max_addr or seg.vaddr <= base_addr < seg.vaddr + seg.memsize:
                    ...
                    # if there is data before the beginning of the child or after
                    # the end, make new artificial segments for it
                    ...
                    # we're going to enumerate the contents of the core segment. at each point we
                    # find the relevant child backer. if this skips any content, inject a backer into the child.
                    ...
                    while cursor < seg.filesize:
                        # use filesize and not memsize so we don't overwrite stuff
                        # with zeroes if it's omitted from the core
                        child_cursor = cursor + seg.vaddr - base_addr
                        try:
                            child_offset, child_backer = next(obj.memory.backers(child_cursor))
                        except StopIteration:
                            # is this right? is there any behavior we need to account for in the
                            # case that there is somehow no backer past a point mapped by the core?
                            break
