...
class GenericIRelativeReloc(Relocation):
    def relocate(self, solist, bypass_compatibility=False):
        if self.symbol.type == Symbol.TYPE_NONE:
            self.owner_obj.irelatives.append((self.owner_obj.mapped_base + self.addend, self.addr))
            self.resolve(None)
            return True
        ...
        self.owner_obj.irelatives.append((self.resolvedby.mapped_base, self.addr))
